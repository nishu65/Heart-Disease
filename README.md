📊 Dataset Structure

| Column Name | Description |
|---|---|
| 🎂 Age | Age of the patient |
| 👤 Sex | Gender (1 = Male, 0 = Female) |
| 🫀 Chest Pain Type | Type of chest pain reported |
| 🩺 Resting BP S | Resting blood pressure |
| 🧪 Cholesterol | Serum cholesterol in mg/dl |
| 🍬 Fasting Blood Sugar | Blood sugar > 120 mg/dl (1 = true; 0 = false) |
| 📉 Resting ECG | Resting electrocardiogram results |
| 💓 Max Heart Rate | Maximum heart rate achieved |
| 🏃‍♂️ Exercise Angina | Exercise-induced chest pain (1 = yes; 0 = no) |
| 🧗 Oldpeak | ST depression induced by exercise |
| ⛰️ ST Slope | The slope of the peak exercise ST segment |
| 🎯 Target | Heart Disease (1 = Yes, 0 = No) |

📋 First 5 Records

| Age | Sex | Chest Pain | BP | Cholesterol | FBS | ECG | Max HR | Angina | Oldpeak | ST Slope | Target |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 40 | 1 | 2 | 140 | 289 | 0 | 0 | 172 | 0 | 0.0 | 1 | 0 |
| 49 | 0 | 3 | 160 | 180 | 0 | 0 | 156 | 0 | 1.0 | 1 | 1 |
| 37 | 1 | 2 | 130 | 283 | 0 | 1 | 98 | 0 | 0.0 | 1 | 0 |
| 48 | 0 | 4 | 138 | 214 | 0 | 0 | 108 | 1 | 1.5 | 2 | 1 |
| 54 | 1 | 3 | 150 | 195 | 0 | 0 | 122 | 0 | 0.0 | 1 | 0 |

**Model Performance:** MAE = 0.16806722689075632

Summary: The file contains 1,190 rows 📂 and 12 columns 📏 focused on heart health data. Ready for analysis! 🚀
